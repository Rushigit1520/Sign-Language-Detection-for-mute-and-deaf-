...
**Recent Updates:**
- **Code Refactoring**: Extracted brittle `if-else` landmark checks into a modular `HeuristicClassifier`.
- **Temporal Prediction Smoothing**: Implemented a 5-frame prediction buffer to prevent the text from flickering/jumping.
- **Modernized GUI**: Upgraded the Tkinter interface to a sleek dark-themed design with properly padded, stylized controls.
- **Robust Pathing**: Eliminated hardcoded absolute paths to make the project portable across different machines.
